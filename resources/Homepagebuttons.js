...
function play_pause_set_icon(elem) {
	...
	if (!force_bg_pause) {
		pause_bg_icon.setAttribute("src", "//plugins/guraClient/assets/Icon/pause_button.png")
	}
	else {
		pause_bg_icon.setAttribute("src", "//plugins/guraClient/assets/Icon/play_button.png")
	}
}
//___________________________________________________________________________//
...
function audio_play_pause() {
	...
	let wallpaperaudio = document.getElementById("gura-bg")
...
function play_pause_set_icon_audio(elem) {
	...
	if (!force_audio_pause) {
		pause_audio_icon.setAttribute("src", "//plugins/guraClient/assets/Icon/audio.png")
	}
	else {
		pause_audio_icon.setAttribute("src", "//plugins/guraClient/assets/Icon/mute.png")
	}
...
function next_wallpaper() {
	...
	guraBg.classList.add("webm-hidden");
	wallpapers.push(wallpapers.shift())
	document.querySelector(":root").classList.add(wallpapers[0].replace(/\.[a-zA-Z]+$/, '-vars'))
	setTimeout(function () {
		guraBg.src = `//plugins/guraClient/assets/Backgrounds/${wallpapers[0]}`
		gura_play_pause()
		guraBg.classList.remove("webm-hidden");
	}, 500);
}
//___________________________________________________________________________//
...
function create_webm_buttons() {
	...
	nextBgIcon.setAttribute("src", "//plugins/guraClient/assets/Icon/next_button.png")
	prevBgIcon.setAttribute("src", "//plugins/guraClient/assets/Icon/prev_button.png")
...
//___________________________________________________________________________//
let homepagebutton = {
    gura_play_pause: gura_play_pause,
